...
export function renderDag(project: ProjectState, layer: Layer) {
    const layerModules = layer.moduleIds
    const allIds = [...layerModules, ...layer.children.map((l) => l.uid)]
    function getStartingEntityId(connection: Connection) {
    ...
    const data = allIds.map((uid) => {
        if (layerModules.includes(uid)) {
            const connections = project.main.connections.filter(
                (connection) => {
                    return connection.end.moduleId == uid //getConnectedEntity(connection, 'end')
                },
            )
            return {
                id: uid,
